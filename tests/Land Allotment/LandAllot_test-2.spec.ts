...
import { LandAllotmentPage } from '../../pages/admin/LandAllotment.page';

test('Land Allotment Application Flow', async ({ page }) => {
  const landAllot = new LandAllotmentPage(page);

  await landAllot.navigateToLogin();
  await landAllot.login();
  await landAllot.startApplication();
  await landAllot.fillForm();
  await landAllot.submitForm();

  // Example: Check for a success URL or confirmation message (update as needed)
  // await expect(page).toHaveURL(/success|dashboard/i);
});
...
//   await page.getByRole('textbox', { name: 'Parent Company *' }).fill('Sys Soln');
//   await page.getByRole('textbox', { name: 'Name (Focal point 1) *' }).click();
//   await page.getByRole('textbox', { name: 'Name (Focal point 1) *' }).fill('dos1');
//   await page.getByRole('textbox', { name: 'Designation (Focal point 1) *' }).click();
//   await page.getByRole('textbox', { name: 'Designation (Focal point 1) *' }).fill('sed-dos');
...
//   await page.getByRole('textbox', { name: 'Parent Company Address *' }).fill('1,da,road-9,kk9,New orland');
...
//   await page.getByText('Plot type DevelopedUndevelopedSpecialized Infrastructure').click();
